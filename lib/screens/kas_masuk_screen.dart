...
class _KasMasukScreenState extends State<KasMasukScreen> {
  ...
  void baru() async {
    await showDialog(
      ...
      return Dialog(
          child: BaruDialog(
            onSimpan: Provider.of<KasProviders>(context, listen: false).tambah,
          ),
      );
     },
   );
  ...
  void onItemTap (Kas kas) async {
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Yakin ingin menghapus kas masuk tsb?'),
        actions: [
          ElevatedButton(
            onPressed: () {
             Provider.of<KasProviders>(context, listen: false).hapus(kas);
          ...
          body: Consumer<KasProviders>(
            builder: (context, value, child) {
              ...
              return Column(
                children: [
                  CardKasInfo(
                    items: items, jenis: JenisKas.kasMasuk),
                    Flexible(
